...
function materializeBle(source) {
  ...
  replace('constexpr uint8_t DIAGNOSTICS_VERSION = 2;','constexpr uint8_t DIAGNOSTICS_VERSION = 5;','Chakshu hardware diagnostics version');
  replace('  uint8_t value[48] = {};','  uint8_t value[112] = {};','Chakshu hardware diagnostics size');
  replace('  diagnosticsCharacteristic->setValue(value,sizeof(value));',
    '  ChakshuLink::append(value,deviceConnected.load(),streamingEnabled.load()&&!recoveryWaiting.load(),chakshuAudioSubscribed.load(),millis());\n  value[84]=digitalRead(TOUCH_INPUT_PIN)==TOUCH_ACTIVE_LEVEL?1:0;value[85]=touchStableState?1:0;value[86]=batteryAvailable.load()?1:0;value[87]=batteryPercent;\n  value[88]=batteryAdcMillivolts&255;value[89]=batteryAdcMillivolts>>8;value[90]=batteryAdcRaw&255;value[91]=batteryAdcRaw>>8;value[92]=batteryMillivolts&255;value[93]=batteryMillivolts>>8;\n  value[94]=uint8_t(lastLedPattern>>16);value[95]=uint8_t(lastLedPattern>>8);value[96]=uint8_t(lastLedPattern);value[97]=TOUCH_INPUT_PIN;value[98]=BATTERY_ADC_PIN;value[99]=RGB_LED_PIN;\n  put32le(value+100,touchTransitions.load());put32le(value+104,touchActions.load());value[108]=touchLastHoldMs.load()&255;value[109]=touchLastHoldMs.load()>>8;value[110]=remoteStandby?1:0;value[111]=uint8_t(deviceState);\n  diagnosticsCharacteristic->setValue(value,sizeof(value));','Append link and hardware evidence');
