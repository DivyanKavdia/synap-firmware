...
| Recovery overflow vs partial transmission | If capture evicted the current ring head during retries, the next attempt selected newer PCM and reset fragment progress. A persistently slow link could then stop completing frames. | Retain one in-flight PCM frame until completion or owner/replay change, independently of ring eviction. A native test overwrites the ring between every fragment and verifies exact PCM and acknowledgement behavior. |
...
The supplied build-1235 screenshots show only nine seconds received after 36 seconds elapsed, followed by no arriving audio. This establishes a delivery shortfall. Ring-overflow starvation is reproduced in native tests, but the screenshots alone do not establish whether the underlying low delivery rate comes from PDM capture, radio/controller pressure or the browser. Exported capture/drop/reject counters are needed to distinguish those causes.
