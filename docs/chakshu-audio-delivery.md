...
The recovery transmitter retains one complete in-flight PCM frame across partial-send retries. Ring overflow may evict that frame from recovery history, but cannot replace its remaining fragments. Once sent, the ring acknowledges the sequence only if it is still at its cursor; no newer frame is skipped. Connection, recording generation and replay changes invalidate this held frame. The overflow regression forces new capture to replace the ring contents between each fragment and verifies the original PCM still completes. This prevents permanent fragment starvation; it does not manufacture missing samples or prove sufficient radio throughput.
