...
const {validate,createManifest,canonicalManifest,target,version,slotSize}=require('../tools/release.cjs');
...
test('test and production manifests have isolated immutable URLs',()=>{
  ...
  assert.equal(testManifest.channel,'test');assert.match(testManifest.url,/\/ota-test\/builds\/1001-/);
  assert.equal(prod.channel,'production');assert.match(prod.url,/\/ota-releases\/builds\/1001-/);
  assert.equal(prod.schema,2);assert.equal(prod.signing,undefined);
