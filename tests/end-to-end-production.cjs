...
test('final production S3 source matches the shipped audio, touch, power and OTA contract',()=>{
  ...
  assert.match(s3,/MIC_START_ATTEMPTS=3[\s\S]*?attempt<=MIC_START_ATTEMPTS/,'I2S initialization must retry after standby/deep-sleep reinitialization');
  ...
  assert.match(s3,/microphoneValidated=startMicrophone\(\);\n#endif\n  applyCpuPowerProfile\(false\);/,'awake boot must leave the microphone clocked instead of immediately tearing I2S down');
  assert.match(s3,/reason==ErrorCode::AUDIO_SOURCE_FAILED && microphoneReady\) stopMicrophone\(\)/,'only a genuine audio-source failure should tear down I2S during an ordinary awake session');
  assert.doesNotMatch(s3,/microphoneValidated=startMicrophone\(\);[\s\S]{0,80}if \(microphoneValidated\) stopMicrophone\(\)/,'boot validation must not power-cycle I2S');
  const stopCase=s3.slice(s3.indexOf('case CMD_STOP:'),s3.indexOf('break;',s3.indexOf('case CMD_STOP:'))+6);
  assert.doesNotMatch(stopCase,/stopMicrophone\(\)/,'normal STOP must keep I2S alive for the next take');
  assert.match(s3,/enterRemoteStandby[\s\S]*?stopMicrophone\(\)/,'explicit BLE standby may power the microphone down');
  assert.match(s3,/enterDeepSleep[\s\S]*?stopMicrophone\(\)/,'deep sleep must still shut the microphone down');
  ...
  assert.doesNotMatch(s3,/TOUCH_START_HOLD_MS = 2000/,'two-second start gesture must not survive the final preparation chain');
  assert.match(s3,/TOUCH_SLEEP_HOLD_MS = 5000/);assert.match(s3,/TOUCH_TAP_MIN_MS = 80/);assert.match(s3,/TOUCH_TAP_MAX_MS = 450/);
  assert.match(s3,/double tap -> START/);assert.match(s3,/double tap -> STOP/);assert.match(s3,/single touch wake/);
  assert.doesNotMatch(s3,/hold for 5 seconds to stay awake/,'deep-sleep wake is a single touch');
  assert.match(s3,/STANDBY=4/);assert.match(s3,/CMD_STANDBY = 0x03/);assert.match(s3,/CMD_WAKE = 0x04/);
  assert.match(s3,/publishPowerEvent\(POWER_STATE_DEEP_SLEEP\)/,'PWA gets last-known deep sleep state before BLE goes away');
  const poll=s3.slice(s3.indexOf('void pollTouchControl() {'),s3.indexOf('\n}\n',s3.indexOf('void pollTouchControl() {'))+3);
  assert.doesNotMatch(poll,/publishRememberEvent|TOUCH_STOP_MIN_MS|TOUCH_STOP_MAX_MS|TOUCH_START_HOLD_MS/,'legacy touch gestures must be absent');
...
test('C3 materialization preserves the same recording and power protocol with target-safe hardware differences',()=>{
  ...
  assert.match(c3,/microphoneValidated=startMicrophone\(\);\n#endif\n  applyCpuPowerProfile\(false\);/);
  assert.doesNotMatch(c3,/TOUCH_START_HOLD_MS = 2000/);assert.match(c3,/TOUCH_SLEEP_HOLD_MS = 5000/);
  assert.match(c3,/double tap -> START/);assert.match(c3,/single touch wake/);assert.match(c3,/STANDBY=4/);
...
test('release workflow compiles the exact power-controls source and real microphones for both targets',()=>{
  ...
  assert.match(workflow,/patch-power-controls-v2\.cjs/);
