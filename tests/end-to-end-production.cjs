...
test('final production S3 source matches audio, touch, low-power and OTA contract',()=>{
  ...
  assert.match(s3,/bool microphoneRecoveryUsed=false[\s\S]*?empty I2S reads; restarting capture driver[\s\S]*?stopMicrophone\(\)[\s\S]*?startMicrophone\(\)/);
  assert.match(s3,/microphoneValidated=startMicrophone\(\);\n  if \(microphoneValidated\) stopMicrophone\(\);/,'boot hardware validation must not leave I2S running');
  assert.match(s3,/if \(microphoneReady\) \{ vTaskDelay\(pdMS_TO_TICKS\(90\)\); stopMicrophone\(\); \}/,'ordinary STOP must power down I2S');
  assert.match(s3,/remote standby -> awake; microphone remains off until START/);
  assert.match(s3,/enterRemoteStandby[\s\S]*?stopMicrophone\(\)/);
  assert.match(s3,/enterDeepSleep[\s\S]*?stopMicrophone\(\)/);
  assert.match(s3,/uint8_t emptyReads=0[\s\S]*?\+\+emptyReads < 3/);
  ...
  assert.doesNotMatch(s3,/DeviceState::STANDBY/,'current PWA rejects status states above 3');
  assert.match(s3,/setDeviceState\(DeviceState::CONNECTED_IDLE, ErrorCode::NONE\)[\s\S]*?POWER_STATE_STANDBY/);
...
test('C3 materialization preserves the same recording and power contract',()=>{
