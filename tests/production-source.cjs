...
test('production battery, identity and OTA contracts remain in the reviewed source',()=>{
  ...
  assert.match(source,/BATTERY_CAL_CELL_MV = 4130u/);
  ...
  assert.match(source,/void publishBatteryEvent\(\)[\s\S]*?controlCharacteristic->notify\(\);[\s\S]*?vTaskDelay\(pdMS_TO_TICKS\(20\)\)[\s\S]*?updateStatusCharacteristic\(false\)/);
