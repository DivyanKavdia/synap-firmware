...
int main(){
 assert(onTime(3000)==50);
 deviceState=DeviceState::CONNECTED_IDLE;assert(onTime(8000)==30);
 deviceState=DeviceState::STREAMING;assert(onTime(1000)==100);
 remoteStandby=true;assert(onTime(8000)==0);
 remoteStandby=false;sleepPending=true;assert(onTime(3000)==0);
 sleepPending=false;updating=true;assert(onTime(1400)==110);
 updating=false;remoteStandby=true;updateStatusLed(false);const int before=writes;
