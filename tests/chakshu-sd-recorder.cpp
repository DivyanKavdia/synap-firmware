...
namespace ChakshuCamera {bool ready=true;bool configure(bool){return true;}}
...
namespace ChakshuMedia {
 enum {NO_SD=3,NO_CAMERA=4,NO_MIC=5,NO_SPACE=6,IO_ERROR=7,CAPTURE_ERROR=8};
 struct Snapshot {char path[64]{};uint32_t bytes=0,audioMs=0,frames=0,droppedFrames=0;uint8_t state=0,error=0,progress=0;};
}
// INSERT BUFFERS
// INSERT RECORDER
...
ChakshuMedia::Snapshot record(bool video){
 ChakshuMedia::Snapshot s;ChakshuRecorder::record(s,video,stop,progress);
...
int main(){
 ...
 reset();s=record(false);assert(!s.error && s.frames==0);verifyAudio(s);
 ...
 for(int failed:{1,2}){reset();failTask=failed;s=record(true);assert(s.error==8);}
 for(int failed:{1,2}){reset();failAllocation=failed;s=record(true);assert(s.error==8 && !created);}
 ...
 reset();cameraReady=false;s=record(true);assert(s.error==8);
