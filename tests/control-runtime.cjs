...
test('BLE link changes survive queue pressure and stale commands cannot starve maintenance',()=>{
  ...
  const linkConstants=source.slice(source.indexOf('constexpr uint16_t BLE_MIN_INTERVAL'),source.indexOf('constexpr uint16_t MAX_AUDIO_PAYLOAD_BYTES'));
  ...
  assert.match(nativeTest(fixture.replace('// INSERT LINK CONSTANTS',linkConstants).replace('// INSERT CONTROL TYPES',types).replace('// INSERT START',start).replace('// INSERT CALLBACKS',callbacks).replace('// INSERT CONTROL TASK',task)),/PASS link recovery/);
});
