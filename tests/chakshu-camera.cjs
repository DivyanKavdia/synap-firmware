...
test('camera mode changes preserve VGA photos and reject failed sensor changes without leaking a frame',()=>{
 ...
 const capture=transfer.slice(transfer.indexOf('uint8_t captureFrame('),transfer.indexOf('uint8_t catalogue()'));
...
sensor_t sensor{change};sensor_t* esp_camera_sensor_get(){return sensorPresent?&sensor:nullptr;}
...
${capture}
int main(){
 assert(captureFrame(true)==0&&mode==FRAMESIZE_QVGA&&changes==1&&returns==2);
 assert(bufferSize==sizeof(jpeg)&&memcmp(buffer,jpeg,sizeof(jpeg))==0&&buffer!=jpeg);
 assert(captureFrame(true)==0&&changes==1&&returns==4);
 assert(captureFrame()==0&&mode==FRAMESIZE_VGA&&changes==2&&returns==6);
