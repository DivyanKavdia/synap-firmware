...
// Both OTA slots retain a complete code/model pair; no model partition is rewritten.
...
LoadResult inflate(const uint8_t* input,size_t length,uint8_t* output,size_t capacity) {
  ...
  tinfl_init(decoder);
  size_t inputAt=0,outputAt=0;LoadResult result=INVALID;
  for(;;){
    ...
    if(state==TINFL_STATUS_DONE){if(inputAt==length&&outputAt==capacity)result=LOADED;break;}
